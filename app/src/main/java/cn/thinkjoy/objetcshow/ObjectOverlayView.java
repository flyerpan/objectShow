...
public class ObjectOverlayView extends View {
    ...
    public ObjectOverlayView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initialize();
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ObjectOverlayView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (objectList != null && objectList.size() > 0 && isStart) {
            ...
            for (ObjectInfo object : objectList) {
                ...
                canvas.save();
//                canvas.drawText(object.getAlias(), (left + right) / 2, (top + bottom) / 2, textPaint);
